...
def fetch_process_and_save(task: dict):
    ...
    try:
        if source == "sklearn":
            ...
            dataset_name = dataset.metadata.get('slug', dataset.metadata.get('name', str(ds_id)))
        ...
        return {
            'dataset_name': failed_name, 'source': source, 'n_samples': 0, 'n_features': 0,
            'n_classes': 0, 'status': f'Failed: Could not fetch - {e}', 'saved_file': "N/A"
        }

    # --- 2. Preprocessing ---
    if df is None:
        return {
            'dataset_name': sanitized_name, 'source': source, 'n_samples': 0, 'n_features': 0,
            'n_classes': 0, 'status': 'Failed: Dataframe was empty after fetch.', 'saved_file': "N/A"
        }
    ...
    if processed_df is not None:
        ...
        return {
            'dataset_name': sanitized_name, 'source': source, 'n_samples': n_samples,
            'n_features': n_features, 'n_classes': n_classes, 'status': 'Success',
            'saved_file': f"{sanitized_name}.csv"
        }
    else:
        return {
            'dataset_name': sanitized_name, 'source': source, 'n_samples': 0, 'n_features': 0,
            'n_classes': 0, 'status': f'Failed: {status}', 'saved_file': "N/A"
        }
...
def main():
    ...
    openml_ids = [
        ...
        1464, 40966, 41146, 40996, 40978, 41138, 1462
    ]
    uci_ids = [
    ...
    with ProcessPoolExecutor() as executor:
        ...
        future_to_task = {executor.submit(fetch_process_and_save, task): task for task in tasks}
        
        # Use tqdm for a progress bar as futures complete
        for future in tqdm(as_completed(future_to_task), total=len(tasks), desc="Processing datasets"):
            try:
                ...
                summaries.append({
                    'dataset_name': failed_name, 'source': task['source'], 'n_samples': 0, 'n_features': 0,
                    'n_classes': 0, 'status': f'Failed: Critical error - {e}', 'saved_file': "N/A"
                })

    # --- Save final summary report ---
    summary_df = pd.DataFrame(summaries).sort_values(by=['source', 'dataset_name']).reset_index(drop=True)
